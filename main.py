...
def identify_regimes(
    ...
    """
    Identify market regimes using clustering on hidden states
    ...
    Returns:
        Tuple[Any, int]: K-means clustering model and bull regime cluster ID
    """
    logger = logger or logging.getLogger('regime_mamba')
    logger.info("Extracting hidden states from validation data")
    ...
    try:
        ...
        logger.error(f"Error identifying regimes: {str(e)}")
        raise
...
def analyze_costs(
    ...
    """
    Analyze impact of different transaction cost levels
    ...
    Returns:
        pd.DataFrame: Cost analysis results
    """
    logger = logger or logging.getLogger('regime_mamba')
    logger.info("Analyzing impact of different transaction cost levels")
    
    try:
        # Run cost analysis
        cost_analysis_df = analyze_transaction_cost_impact(
            model, valid_loader, test_loader, config, kmeans, bull_regime,
            save_path=paths['cost_analysis_plot']
        )
        
        # Save results
        cost_analysis_df.to_csv(paths['cost_analysis'], index=False)
        
        return cost_analysis_df
    except Exception as e:
        logger.error(f"Error analyzing transaction costs: {str(e)}")
        logger.error("Skipping transaction cost analysis")
        return None
...
def main():
    ...
    try:
        ...
        cost_analysis_df = analyze_costs(
            model, valid_loader, test_loader, config, kmeans, bull_regime, paths, logger
        )
